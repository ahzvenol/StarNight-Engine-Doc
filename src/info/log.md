...
或许可以想到快进和读档之间存在某种一致性，快进到存档位置相当于进行了读档，这种方式称为演算式。

根据存档的字面含义，则可以想到存储当前的舞台状态，通过恢复它完成读档，这种方式称为存储式。

演算式从剧本第 0 幕快进到存档幕，在读档过程中，命令可能会产生不必要的开销，演算式需要消除其中不可接受的开销。

外部输入，如用户选择的选项，并不编码在剧本中，使用演算的方式也需要存储它们，并在读档过程中填入剧本。

演算式的缺点在于从第 0 幕开始计算状态，剧本越长读档速度就越慢，过长的剧本需要通过拆分场景来优化性能。

存储式需要通过额外的状态表，记录命令起始或终止时刻的状态，再通过它重新恢复舞台状态。

因为读档通常预期重新演出存档幕，同时恢复命令的中间状态和后续操作相比单纯恢复始末状态更加复杂。

存储式的缺点在于状态表与舞台状态是分离的，每次新增舞台元素，都需要对应的实现它在状态表中的存储和恢复。
